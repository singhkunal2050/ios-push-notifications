...
//   debugger;
//   const title = data.title;
//   // const options = data.notificationOptions;
//   const options = {
//     body: data.notificationOptions.body || "",
//     requireInteraction: data.notificationOptions.requireInteraction || false,
//     icon: "http://localhost:8000/android.ico",
//     badge: "http://localhost:8000/icon.png",
//   };
...
//       (action) => action.action === clickedAction
//     );
//     if (action && action.deepLink) {
//       // Open the deep link associated with the action
//       event.waitUntil(clients.openWindow(action.deepLink));
//     }
//   } else {
//     // Handle generic click on the notification (not on an action button)
//     event.waitUntil(clients.openWindow("https://your-default-url.com"));
//   }
// });
...
function onClick(event, redirectPath, notificationData) {
  ...
  if (event.action === "action1") {
    ...
    if (
      typeof notificationData.notificationOptions.actions[0].deepLink !==
      "undefined"
    ) {
      finalDeepLink +=
        "&r=" +
        encodeURIComponent(
          notificationData.notificationOptions.actions[0].deepLink
        );
      silentRequest = false;
    }
    ...
    if (
      typeof notificationData.notificationOptions.actions[1].deepLink !==
      "undefined"
    ) {
      finalDeepLink +=
        "&r=" +
        encodeURIComponent(
          notificationData.notificationOptions.actions[1].deepLink
        );
      silentRequest = false;
    }
